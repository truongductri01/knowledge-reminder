...
class SignUpView(APIView):
    ...
    def post(self, request, format=None):
        ...
        return Response({"No Content": "Enter users info to sign up"}, status=status.HTTP_204_NO_CONTENT)
    ...
    def post(self, request, format=None):
        ...
        if user.exists():
            if password == user[0].password:
                ...
                data = {"user_name": self.request.session["user_name"], 'logged_in': self.request.session["logged_in"]}
                return JsonResponse(data, status=status.HTTP_200_OK)       

            return Response({"Invalid": "Invalid username or password"}, status=status.HTTP_403_FORBIDDEN)       
        return Response({"Bad Request": "Invalid username or password"}, status=status.HTTP_403_FORBIDDEN)

class UserLoggedIn(APIView):
    def get(self, request, format = None):
        if not self.request.session.exists(self.request.session.session_key):
            self.request.session.create()
        if self.request.session.has_key("user_name") and self.request.session.has_key("logged_in"):
            data = {"user_name": self.request.session["user_name"], 'logged_in': self.request.session["logged_in"]}
            return JsonResponse(data, status=status.HTTP_200_OK) 
...
class LogOutView(APIView):
    def post(self, request, format=None):
        if "user_name" in self.request.session and "logged_in" in self.request.session:
            self.request.session.pop("user_name")
